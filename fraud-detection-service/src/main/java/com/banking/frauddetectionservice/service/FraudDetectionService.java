...
public class FraudDetectionService {
    ...
    private FraudCheckResult performFraudChecks(String accountNumber, BigDecimal amount, BigDecimal senderBalance){
        ...
        if(isVelocityExceeded(accountNumber)){
            return new FraudCheckResult(true,"Too many transactions in 60 seconds "+" - Velocity limits exceeded");
        }

        // pattern 2. Amount Check
        if(isAmountSuspicious(accountNumber, amount)){
            return new FraudCheckResult(true, "Unusual transaction amount " + " - exceeds 3x of your average");
        }

        // pattern 3. Balance check
        if(senderBalance.compareTo(BigDecimal.ZERO) > 0 && isBalanceFailed(senderBalance,amount)){
    ...
    private boolean isVelocityExceeded(String accountNumber){
        ...
        return count != 0 && count>maxTransactionPerMinute;
    }
    ...
    private boolean isBalanceFailed(BigDecimal senderBalance, BigDecimal amount){
